...
make_metab_df <- function(){
  ...
  for (i in 1:dim(sub.df)[1]){
    ...
    metab <- sub.df$`Most associated metabolite or ratio`[i] #s6.df$`Most associated metabolite or ratio`[i]
    pcg <- sub.df$`Predicted causal gene`[i]#s6.df$`Predicted causal gene`[i]
